...
// See safe_i2c.h for documentation
bool SafeI2C::get_mutex(const char* const func) {
    bool result = true;
    if (xSemaphoreTake(this->i2c_mutex, MAX_MUTEX_WAIT_TICKS) != pdTRUE) {
        ESP_LOGE(LOG_TAG, "Failed to take the I2C mutex in function %s", func);
        result = false;
    }
    return result;
}

// See SafeI2C.h for documentation
void SafeI2C::release_mutex(const char* const func) {
    if (xSemaphoreGive(this->i2c_mutex) != pdTRUE) {
        ESP_LOGE(LOG_TAG, "Failed to release the I2C Mutex in function %s",
                 func);
    }
}